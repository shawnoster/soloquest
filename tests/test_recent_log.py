...
class TestRecentLog:
    ...
    def test_recent_log_includes_move_entries(self):
        ...
        entries = [
            ...
            LogEntry(
                kind=EntryKind.MOVE, text="**Strike** | d6(5)+iron(3) = 8 vs [4, 7] → STRONG HIT"
            ),
        ...
        printed_outputs = [
            str(call[0][0]) if call[0] else "" for call in mock_console.print.call_args_list
        ]
        move_printed = any(
            "Strike" in output and "STRONG HIT" in output for output in printed_outputs
        )
    ...
    def test_recent_log_includes_oracle_entries(self):
        ...
        oracle_printed = any(
            "Oracle" in output and "Advance" in output for output in printed_outputs
        )
    ...
    def test_recent_log_all_entry_types_together(self):
        ...
        entries = [
            ...
            LogEntry(
                kind=EntryKind.MOVE, text="**Face Danger** | d6(3)+wits(3) = 6 vs [5, 8] → WEAK HIT"
            ),
